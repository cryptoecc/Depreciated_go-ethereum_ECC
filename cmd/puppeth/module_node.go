...
const owner = '0x575f4b87a995b06cfd2a7d9370d1fb2bc710fdc9';

const delegator = '0x7c23a65d25cc8486557c988f3c9a68c5b595dd10';
const delegatee = '0x575f4b87a995b06cfd2a7d9370d1fb2bc710fdc9';
...
web3.personal.unlockAccount(owner, "iise1542", 0);
web3.personal.unlockAccount(delegatee, "iise1542", 0);
...
const main = async() => {
  ...
  await stamina.getDelegatee(delegator);
  await stamina.deposit(delegatee, {from: owner, value: 1e18, gas: 2e6});

  await web3.eth.getTransactionCount(delegator);
  await stamina.getStamina(delegatee);
};
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	...
	echo $'geth --networkid {{.NetworkID}} --cache 512 --port {{.Port}}  --rpc --rpcapi personal,eth,net,web3,admin,txpool --rpcport 8545 --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> geth.sh
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(staminaScript)).Execute(scriptfile, map[string]interface{}{
		...
		"OwnerPassword": config.ownerPass,
		"DelegateePassword": config.delegateePass,
	})
